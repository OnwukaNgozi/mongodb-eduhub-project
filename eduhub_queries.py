...
# Assuming a rating field exists in a submissions or courses collection
...
# Assuming isCompleted is in enrollments
db.enrollments.aggregate([
    {
        "$group": {
            "_id": "$courseId",
...
db.courses.aggregate([
    ...
    },
    {
        "$group": {
            "_id": "$instructorId",
            "studentsTaught": {"$sum": {"$size": "$enrollments"}}
        }
    }
])
# %%
# Average course rating per instructor
# Assumes rating field in courses
db.courses.aggregate([
    {
        "$group": {
            "_id": "$instructorId",
...
# Assumes each enrollment pays the course.price
db.courses.aggregate([
    {
        "$lookup": {
            "from": "enrollments",
...
# Lesson completion = average progress per student per course
db.enrollments.aggregate([
    {
        "$group": {
            "_id": "$studentId",
