...
# 3.1.3.Enroll the student in the course
enrollment = {
    "enrollmentId": "E016",
    "studentId": "U021",
    "courseId": "C009",
...
# %%
# 3.2.1. Query to find all active students
...
category = "Computer Science"  # Example category
...
top_performing_students = db.submissions.aggregate([
    {
        "$group": {
            "_id": "$studentId",
            "avgScore": {"$avg": "$score"},
...
completion_rate_by_course = db.enrollments.aggregate([
    {
        "$group": {
            "_id": "$courseId",
            "totalEnrolled": {"$sum": 1},
    ...
    },
    {
        "$project": {
            ...
            "completionRate": {
                "$cond": [
                    {"$gt": ["$totalEnrolled", 0]},
                    {"$divide": ["$completedCount", "$totalEnrolled"]},
                    0
                ]
            },
            "totalEnrolled": 1,
...
pipeline = [
    ...
    },
    {
        "$group": {
            ...
            "avgRating": {"$avg": "$allSubmissions.rating"}
        }
    },
    {
        "$lookup": {
